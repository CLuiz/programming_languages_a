...
(*
f: T1 -> T2 [msut be a function; all functions take 1 arg]
x : T1

y : T3
z : T4

T1 = T3 * T4 [else apttern amtch will not type check]
T3 = int [abs has type int -> int]
T4 = int [ because we added z to an int]
sp T1 = int * int
sp (abs y) + z : int, so let expression : int, so body : int
T2 = int
f : int * int * -> int
...
T3 = int list [because x: T3 and we add x to something]
